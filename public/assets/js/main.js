...
function generateImportanceGrid(traits) {
  ...
  headerRow.className = 'd-flex align-items-center';
  headerRow.innerHTML = `
  <div class="col-4"></div>
  </br>
  <div class="col-8 d-flex justify-content-between">
            <div class="text-center" style="padding-right: 20px">1</div>
            <div class="text-center" style="padding-right: 20px">2</div>
            <div class="text-center" style="padding-right: 20px">3</div>
            <div class="text-center" style="padding-right: 20px">4</div>
            <div class="text-center" style="padding-right: 20px">5</div>
        </div>
  
  `;
  importanceGrid.appendChild(headerRow);

  traits.forEach((trait, index) => {
      const traitRow = document.createElement('div');
      traitRow.className = 'd-flex align-items-center mb-3';

      const traitLabel = document.createElement('div');
      traitLabel.className = 'col-4';
      ...
      for (let i = 0; i < 5; i++) {
          ...
          radioInput.name = `importance${index + 1}`;
          radioInput.value = i;
      ...
      importanceGrid.appendChild(traitRow);
  });
}
...
function generateCareerRatings(careers, traits) {
  ...
  careers.forEach((career, careerIndex) => {
      ...
      <div class="col-8 d-flex justify-content-between">
                ...
                <div class="text-center" style="padding-right: 10px">6</div>
                <div class="text-center" style="padding-right: 10px">7</div>
            </div>
...
function collectImptData() {
  IMPORTANCE = [];
  for (let i = 1; i <= TRAITS.length; i++) {
      const selectedValue = document.querySelector(`input[name="importance${i}"]:checked`).value;
      IMPORTANCE.push(selectedValue);
  }
...
function calculateAndDisplayRatings(ratings, importance, careers) {
  ...
  for (let i = 0; i < careers.length; i++) {
      ...
      const weightedScore = careerRatings.reduce((sum, rating, index) => sum + parseInt(rating) * parseInt(importance[index]), 0);
